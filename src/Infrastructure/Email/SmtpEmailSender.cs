...
using CleanArchToDoExample.Domain.Interfaces;
using Microsoft.Extensions.Logging;
using Microsoft.Extensions.Options;

namespace CleanArchToDoExample.Infrastructure.Email;
    ...
    IOptions<MailserverConfiguration> mailserverOptions) : IEmailSender
{
    ...
    public async Task SendEmailAsync(string to, string from, string subject, string body)
    {
        ...
        logger.LogWarning("Sending email to {to} from {from} with subject {subject} using {type}.", to, from, subject,
            this.ToString());
    }
}
